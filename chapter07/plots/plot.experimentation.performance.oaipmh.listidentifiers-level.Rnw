...
tikz('plot.experimentation.performance.oaipmh.listidentifiers-level.tex', width=5.5, height=3.2)
...
ggplot(data=experimentdataset[experimentdataset$variable=="ListIdentifiers",], 
       aes(x=as.factor(Workload), 
           y=value, 
           group=Level)) + 
             geom_line(aes(linetype=Level)) + 
             geom_point(aes(shape=Level), fill="white") + 
