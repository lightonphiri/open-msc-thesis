...
tikz('plot.experimentation.performance.rss-feed.feed-size.tex', width=5.5, height=3.2)
...
experimentdataset <- melt(dataset[dataset$Structure=="h1",], id.vars=c(1,5))
...
experimentdataset["yaba"] <- "Feed size"
...
# rename levels
levels(experimentdataset$Structure)[levels(experimentdataset$Structure)=="h1"] <- paste("1 Level","$\\;\\;\\;$")
levels(experimentdataset$Structure)[levels(experimentdataset$Structure)=="h2"] <- paste("2 Levels","$\\;\\;\\;$")
levels(experimentdataset$Structure)[levels(experimentdataset$Structure)=="h3"] <- paste("3 Levels","$\\;\\;\\;$")


# I have no idea what the plot block below is for... zombified to figure out why
#ggplot(data=experimentdataset, aes(x=as.factor(Workload), y=(value/1000), group=variable, color=variable, shape=variable, linetype=variable)) + geom_line() + geom_point() + scale_shape_manual(values=c(18,4)) + scale_linetype_manual(values=c("solid","dotdash")) + facet_wrap(~ Structure, scales="free_y",ncol=1) + scale_x_discrete(labels=c("100","200","400","800","1.6k","3.2k","6.4k","12.8k","25.6k","51.2k","102.4k","204.8k","409.6k","819.2k","1638.4k")) +  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x))) +  labs(x="",y="")  + theme(legend.title=element_blank(), legend.position=c(0.115,0.92), plot.margin=unit(c(0.2,0.15,0.05,0.05),"cm"), axis.text.x = element_text(angle=90, vjust=0.5))

# create colored plot
##ggplot(data=experimentdataset, aes(x=as.factor(Workload), y=(value/1000), group=variable, color=variable, shape=variable, linetype=variable)) + geom_line() + geom_point() + scale_shape_manual(values=c(18,4,13)) + scale_linetype_manual(values=c("solid","dotdash","longdash")) + scale_x_discrete(labels=c("100","200","400","800","1.6k","3.2k","6.4k","12.8k","25.6k","51.2k","102.4k","204.8k","409.6k","819.2k","1638.4k")) +  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x))) +  labs(x="",y="")  + theme(legend.title=element_blank(), legend.position=c(0.15,0.87), plot.margin=unit(c(0.2,0.15,0.05,0.05),"cm"), axis.text.x = element_text(angle=90, vjust=0.5))
