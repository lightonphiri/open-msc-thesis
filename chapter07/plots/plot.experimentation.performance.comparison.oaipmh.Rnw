...
tikz('plot.experimentation.performance.comparison.oaipmh.tex', width=5.5, height=8.0)
...
# rename approaches
levels(experimentdataset$Source)[levels(experimentdataset$Source)=="FileRepo"] <- paste("Simple repository","$\\;\\;\\;$")
...
ggplot(data=experimentdataset, 
	   ...
	   colour=Source,
           group=Source)) +
             ...
             geom_point(aes(shape=Source), fill="white", size=2.5) + 
