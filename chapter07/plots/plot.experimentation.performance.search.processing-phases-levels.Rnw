...
tikz('plot.experimentation.performance.search.processing-phases-levels.tex', width=5.5, height=7.5)
...
ggplot(data=experimentdataset[experimentdataset$variable!=paste("Total","$\\;\\;\\;$"),], 
       aes(x=as.factor(Workload), 
           y=(value), 
	   colour=Structure,
           group=Structure)) +
             geom_line(aes(linetype=Structure), size=1.5) + 
             geom_point(aes(shape=Structure), size=2.5) + 
