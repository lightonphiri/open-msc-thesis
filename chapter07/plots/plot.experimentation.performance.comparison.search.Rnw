...
tikz('plot.experimentation.performance.comparison.search.tex', width=5.5, height=3.0)

# import datafeeds
dataset <- read.table("../data/experiments-performance-search-comparision.dat", header=TRUE, sep=",")

# convert dataset to molten form for ggplot2
experimentdataset <- melt(dataset, id.vars=c(1))

# add dummy column for faceting
experimentdataset["yaba"] <- "Full text search"

# rename approaches
levels(experimentdataset$variable)[levels(experimentdataset$variable)=="FileBased"] <- paste("Simple repository","$\\;\\;$")
levels(experimentdataset$variable)[levels(experimentdataset$variable)=="Dspace"] <- paste("DSpace","$\\;\\;\\;\\;$")
levels(experimentdataset$variable)[levels(experimentdataset$variable)=="Level3"] <- paste("3 Levels","$\\;\\;\\;$")

# I cannot remember why I have commented plot block below
