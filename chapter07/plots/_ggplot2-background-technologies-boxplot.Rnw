...
require(reshape2)
require(grid)
tikz('ggplot2-background-technologies-boxplot.tex', width=5.9, height=3.5)

dataset <- read.table("../data/experiments-survey-background-technologies-boxplot.dat", header=TRUE, sep=",")
...
ggplot(data=dataset, 
       aes(x=Technology, y=Code, fill=Technology, ymax=4)) + 
         stat_boxplot(geom='errorbar') + 
         geom_boxplot() + 
         geom_point() + 
         coord_cartesian(ylim=c(0,4)) + 
         scale_y_discrete(breaks=c(1, 4), labels=c(paste("$<$"," 1 year"), paste("$>$"," 6 years"))) + 
         scale_x_discrete(labels=element_blank()) + 
         theme(legend.position="none", legend.title=element_blank(), axis.title.x=element_text(vjust=-2), axis.ticks.y=element_blank(), plot.margin=unit(c(0.2,0.15,0.05,0.05), "cm")) + 
